...
def reshape_transform(feats, max_shape=(20, 20), is_need_grad=False):
    ...
    if len(max_shape) == 1:
        ...
        max_shape = (max_shape[0], max_shape[0])
    ...
    for i, feat in enumerate(feats):
        if feat.dim() < 2:
            feats[i] = feat.unsqueeze(0)
            
    max_h = max([im.shape[-2] for im in feats])
    max_w = max([im.shape[-1] for im in feats])
    #max_h = max([feat.shape[-2] for feat in feats])
    #max_w = max([feat.shape[-1] for feat in feats])
    ...
    for feat in feats:
        ...
        interpolate_feat = torch.nn.functional.interpolate(feat.unsqueeze(0), max_shape, mode='bilinear', align_corners=False)
        activations.append(interpolate_feat.squeeze(0))
